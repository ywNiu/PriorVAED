...
def test(model_vae, model_df, net, optimizer, mnist_test, epoch, best_test_loss):
    ...
    with torch.no_grad():  # 这一部分不计算梯度，也就是不放入计算图中去
        ...
        save_checkpoint({
            ...
            'state_dict': model_vae.state_dict(),  # 当前训练过的模型的参数
...
def main():
   ...
   # mnist_test, mnist_train, classes = dataloader(args.batch_size, args.num_worker)
    ...
    # Step 2: 准备工作 : 搭建计算流程
    model_vae = VAE(mask_mtx=mtx_mask, gene_adj_mtx=gene_adj_mtx,z_dim=args.z_dim)  # 生成VAE模型，并转移到GPU上去
    ...
    #optimizer = optim.Adam(model_vae.parameters(), lr=args.lr)  # 生成优化器，需要优化的是model的参数，学习率为0.001
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_index, (x, _) in enumerate(mnist_train):
            ...
            #x = x.view(x.shape[0],1,1,x.shape[1])
            x = x.to(device)
            x_hat, mu, log_var = model_vae(x)  # 模型的输出，在这里会自动调用model中的forward函数
            loss_vae, BCE, KLD = loss_function(x_hat, x, mu, log_var)  # 计算损失值，即目标函数
            img_shape = get_shape()
            ...
            loss_batch.append(loss_vae.item() + loss_df.item())  # loss是Tensor类型
